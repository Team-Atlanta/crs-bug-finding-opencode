...
3. **Verify** — Test each candidate with `libCRS run-pov` on the original build (omit `--rebuild-id`). Check `retcode` (non-zero = crash). Inspect `stdout.log` / `stderr.log` for crash details. Use rebuilt builds only for debugging.
